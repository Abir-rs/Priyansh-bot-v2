...
module.exports.run = async ({ api, event, args }) => {
 ...
 try {
   api.sendMessage("Generating...", threadID, messageID);
    const lim = await axios.get(`https://free-api.ainz-sama101.repl.co/canvas/toanime?imageUrl=${encodeURIComponent(james)}`);
     ...
     api.sendMessage({attachment: fs.createReadStream(pathie)
